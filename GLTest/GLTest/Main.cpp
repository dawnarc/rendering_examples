...
void initScene()
{
	...
	bool rs = prog.CompileShader("shader/01.ads_reflection_model/phong.vert", GLSLShader::GLSLShaderType::VERTEX);
	if (!rs)
	{
		return;
	}
	
	rs = prog.CompileShader("shader/01.ads_reflection_model/phong.frag", GLSLShader::GLSLShaderType::FRAGMENT);
	if (!rs)
	{
		return;
	}
